...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result: any = await (authClient as any).$fetch("/sign-up/email", {
        method: "POST",
  ...
  const handleGoogleLogin = async () => {
    try {
      const result: any = await (authClient as any).$fetch("/sign-in/social", {
        method: "POST",
        ...
        <div className="px-8 py-8">
          <div className="text-center mb-8">
            <Link href="/" className="inline-flex items-center gap-2 mb-6">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center shadow-lg shadow-indigo-200">
              ...
              <span className="font-bold text-slate-900 text-sm tracking-tight">AI Ops <span className="text-indigo-600">TaskFlow</span></span>
            </Link>
            <h1 className="text-2xl font-bold text-slate-900">Create your account</h1>
            <p className="text-sm text-slate-500 mt-1">Start managing AI workflows</p>
          </div>
          <form onSubmit={handleRegister} className="space-y-4">
            <Input id="name" label="Full Name" type="text" placeholder="Arjun Kumar" value={name} onChange={(e) => setName(e.target.value)} required />
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-white px-2 text-slate-400 font-mono">OR</span></div>
          </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">Already have an account? <Link href="/login" className="text-indigo-600 font-semibold hover:text-indigo-700">Sign in</Link></p>
        </div>
      </Card>
    </div>
  )
