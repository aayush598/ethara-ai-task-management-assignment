...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result: any = await (authClient as any).$fetch("/sign-in/email", {
        method: "POST",
        body: { email, password },
      })
      if (result?.redirect === false && result?.user) {
    ...
    <div className="min-h-screen flex items-center justify-center px-5 bg-slate-50">
      ...
      <Card className="w-full max-w-md relative z-10">
        <div className="px-8 py-8">
          ...
          <p className="text-center text-sm text-slate-500 mt-6">Don&apos;t have an account? <Link href="/register" className="text-indigo-600 font-semibold hover:text-indigo-700">Sign up</Link></p>
        </div>
      </Card>
    </div>
  )
